...
__global__ void da_forward_kernel(const float *t, const float *f, float *attention, int num, int chn, int height, int width, int dilation) {
  ...
  if (x < width && y < height && z < 9) {
    for (int batch = 0; batch < num; ++batch) {
      for (int plane = 0; plane < chn; ++plane) {
        ...
        int row = (z / 3 - 1) * dilation;
        int col = (z % 3 - 1) * dilation;
        row = row + y;
        col = col + x;
        if(row >= 0 && col >= 0 && row < height && col < width){
           float _f = f[(batch * chn + plane) * sp + row * width + col];
           attention[(batch * chn + z) * sp + y * width + x] += _t*_f;
           }
        //else{
...
__global__ void da_backward_kernel_t(const float *d_attention, const float *t, const float *f, float *dt,
                                int num, int chn, int height, int width, int dilation) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  int sp = height * width;
  int len = 9;
  ...
  if (x < width && y < height && plane < chn) {
    for (int batch = 0; batch < num; ++batch) {
        ...
        for(int i = 0; i < len; ++i){
            int col = (i % 3 - 1) * dilation + x;
            int row = (i / 3 - 1) * dilation + y;
...
__global__ void da_map_forward_kernel(const float *attention, const float *g, float *out, int num, int chn, int height, int width, int dilation) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  int sp = height * width;
  int len = 9;
  ...
  if (x < width && y < height && z < len) {
    ...
    for (int batch = 0; batch < num; ++batch) {
      for (int plane = 0; plane < chn; ++plane) {
        ...
        int col = (z % 3 - 1) * dilation + x;
        int row = (z / 3 - 1) * dilation + y;
        if(col >= 0 && row >= 0 && col < width && row < height){
            float _g = g[(batch * chn + plane) * sp + row*width + col];
    ...
    for (int batch = 0; batch < num; ++batch) {
      ...
      for (int i = 0; i < len; ++i) {
          ...
          if(col >= 0 && row >= 0 && col < width && row < height){
            ...
            float _attention = attention[(batch * len + i) * sp + row*width + col];
 ...
 * Implementations
 */
extern "C" int _da_forward_cuda(int N, int C, int H, int W, int dilation, const float *t, 
  ...
  da_forward_kernel<<<blocks, threads, 0, stream>>>(t, f, attention, N, C, H, W, dilation);
...
extern "C" int _da_backward_cuda(int N, int C, int H, int W, int dilation, const float *d_attention, const float *t, const float *f, float *dt, float *df, cudaStream_t stream) {
  // Run kernel
  dim3 threads(32, 32);
  int d1 = (W+threads.x-1)/threads.x;
  int d2 = (H+threads.y-1)/threads.y;
  ...
  dim3 blocks(d1, d2, d3);
  // printf("%f\n", dw[0]);
  da_backward_kernel_t<<<blocks, threads, 0, stream>>>(d_attention, t, f, dt, N, C, H, W, dilation);
  da_backward_kernel_f<<<blocks, threads, 0, stream>>>(d_attention, t, f, df, N, C, H, W, dilation);
...
extern "C" int _da_map_forward_cuda(int N, int C, int H, int W, int dilation, const float *attention, const float *g, float *out, cudaStream_t stream) {
  // Run kernel
  dim3 threads(32, 32);
  dim3 blocks((W+threads.x-1)/threads.x, (H+threads.y-1)/threads.y, C);
  da_map_forward_kernel<<<blocks, threads, 0, stream>>>(attention, g, out, N, C, H, W, dilation);
...
extern "C" int _da_map_backward_cuda(int N, int C, int H, int W, int dilation, const float *dout, const float *attention, const float *g, float *d_attention, float *dg, cudaStream_t stream) {
  // Run kernel
  dim3 threads(32, 32);
  int d1 = (W+threads.x-1)/threads.x;
  int d2 = (H+threads.y-1)/threads.y;
  int d3 = H+W;
  dim3 blocks(d1, d2, d3);
  da_map_backward_kernel_w<<<blocks, threads, 0, stream>>>(dout, attention, g, d_attention, N, C, H, W, dilation);

  d3 = C;
  blocks = dim3(d1, d2, d3);
  da_map_backward_kernel_g<<<blocks, threads, 0, stream>>>(dout, attention, g, dg, N, C, H, W, dilation);
